...
{
	public class OnSplitObject : UnityEvent<GameObject, GameObject>
	{
    ...
    public class Splitable : MonoBehaviour, ISplitable
    {
		...
		public OnSplitObject onSplitObject;
        ...
        private void Awake()
        {
			...
			onSplitObject = new OnSplitObject();
        }
        ...
        private void CreateNewObjects()
        {
            ...
            */
            ...
            {
                newGOs[0] = Instantiate(gameObject, null) as GameObject;

				ParticleSystem[] particles = newGOs[0].GetComponentsInChildren<ParticleSystem>();
				if(particles != null)
				{
				...
				//newGOs[0].transform.SetParent(null);
				Transform p = transform.parent;
				if(p != null)
				{
					if(p.GetComponent<Valve.VR.InteractionSystem.Hand>() != null)
					{
				...
				newGOs[0].name = "newPiece";//gameObject.name;
                newGOs[1] = gameObject;
            }
            else
            {
			...
			if(hand != null)
			{
				...
				float dist0 = (newGOs[0].GetComponent<Renderer>().bounds.center - hand.transform.position).magnitude;
				float dist1 = (newGOs[1].GetComponent<Renderer>().bounds.center - hand.transform.position).magnitude;

				if(dist0 < dist1)
				{
					newGOs[0].transform.parent = null;
					newGOs[1].transform.parent = null;   
					hand.AttachObject(newGOs[1]);
				}
				else
				{
					newGOs[0].transform.parent = null;
					newGOs[1].transform.parent = null;   
					hand.AttachObject(newGOs[0]);
				}
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (newMesh != null)
                {
                    ...
                    if (newCollider != null)
                    {
						...
						newCollider.convex = true;// Convex;
			...
			if(onSplitObject != null)
			{
				onSplitObject.Invoke(newGOs[1], newGOs[0]);
				onSplitObject.RemoveAllListeners();
			}
        }
        ...
        protected virtual void PostProcessObject(GameObject go) {
			...
			float vol = bounds.size.x * bounds.size.y * bounds.size.z;

//			Debug.Log("mindim vol " + minDim + " " + vol);

			if(minDim < 0.01 || vol < 0.0001)

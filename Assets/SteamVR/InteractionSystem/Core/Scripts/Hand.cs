...
namespace Valve.VR.InteractionSystem
{
	...
	public class Hand : MonoBehaviour
	{
		...
		public bool spewDebugText = true;
		...
		{
			if(objectToAttach.GetComponent<Throwable>() != null && !objectToAttach.GetComponent<Throwable>().enabled)
				return;
		...
		public void DetachObject( GameObject objectToDetach, bool restoreOriginalParent = true )
		{
			...
			Debug.Log("Couldnt find attached object");
		...
		public void DetachAllObjects()
		{
			...
			for(int i=0; i<attachedObjects.Count; i++)
				DetachObject(attachedObjects[i].attachedObject);
		}
